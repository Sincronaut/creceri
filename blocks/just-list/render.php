...
$parse_bool = static function( $v, $default = false ) {
  ...
  if ( ! is_string( $v ) ) return (bool) $default;
  $s = strtolower( trim( $v ) );
...
foreach ( $cards_raw as $card_raw ) {
  ...
  if ( $heading === '' && empty( $items ) && $cta_text_raw === '' ) continue;
...
if ( ! $show_cta ) { $classes[] = 'has-no-cta'; }
...
$resolve_bullets = static function( $idx, $card, $global_style, $first_col_style, $first_yesno, $second_yesno ) {
  ...
  if ( $yesno === null ) {
    if ( $idx === 0 && $first_yesno !== null )  $yesno = $first_yesno;
    if ( $idx === 1 && $second_yesno !== null ) $yesno = $second_yesno;
  }
  if ( $yesno === false ) return 'none';
 ...
 * ------------------------------------------------------------------------- */
?>
<section<?php if ( $anchor ) echo ' id="' . esc_attr( $anchor ) . '"'; ?> class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>"<?php echo $style_attr; ?>>
