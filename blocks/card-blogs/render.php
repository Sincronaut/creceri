...
function tek_val($arr, $key, $default = null)
{
  return (is_array($arr) && array_key_exists($key, $arr)) ? $arr[$key] : $default;
}
function tek_bool($arr, $key, $default = false)
{
  return (bool)tek_val($arr, $key, $default);
}
function tek_str($arr, $key, $default = '')
{
  $v = tek_val($arr, $key, $default);
...
function tek_get_fallback_image()
{
  return get_stylesheet_directory_uri() . '/assets/images/fallback-image.webp';
}
...
function tek_build_items_from_query(array $attributes): array
{
  ...
  if ($catIds) {
    $query_args['category__in'] = array_map('intval', $catIds);
  }
  if ($excludeIds) {
    $query_args['post__not_in'] = array_map('intval', $excludeIds);
  }
  if ($tax_query) {
  ...
  if ($search !== '') {
    tek_wide_search_add_filters();
  }
  $posts = get_posts($query_args);
  if ($search !== '') {
    tek_wide_search_remove_filters();
  }
  if (!$posts)
    return [];
...
$items = array_map(function ($it) {
  if (trim(tek_str($it, 'image', '')) === '')
    $it['image'] = tek_get_fallback_image();
  return $it;
}, $items);
if ($total_to_show > 0)
...
$visible = function (bool $global, array $item = null, string $hideKey = null) {
  if (!$hideKey || !is_array($item))
    return $global;
  return array_key_exists($hideKey, $item) ? !(bool)$item[$hideKey] : $global;
};
...
$build_read = function (array $item = null) use ($show_read_global, $read_label_global) {
  ...
  if (!$show_read_global || $hide || $url === '' || $label === '')
    return '';
...
$build_tags = function (array $item = null, string $scope = 'item', string $pos = 'top', string $variant = 'outline', string $bg = '#F5E8E6', string $text = '#962E2A', string $border = '#962E2A') use ($visible, $show_category_global) {
  if (!$visible($show_category_global, $item, 'hideCategory'))
    return '';
  $cats = tek_val($item, 'categories', []);
  $cats = is_array($cats) ? array_values(array_filter($cats, fn($c) => is_array($c) && trim(tek_str($c, 'label', '')) !== '')) : [];
  if (!$cats)
    return '';
  ...
  foreach ($cats as $c) {
    $label = esc_html(tek_str($c, 'label', ''));
    $url = esc_url(tek_str($c, 'url', ''));
    $a_cls = 'tag' . ($variant === 'outline' ? ' tag--outline' : '');
    $html .= $url ? '<a class="' . $a_cls . '" href="' . $url . '">' . $label . '</a>' : '<span class="' . $a_cls . '">' . $label . '</span>';
  }
  $html .= '</div>';
  return $html;
};
...
?>
      <article class="feature">
        <div class="feature-media" <?php echo $media_style(tek_str($feature, 'image', tek_get_fallback_image()), $feature); ?>>
          <?php echo $build_tags($feature, 'feature', 'top', 'outline', '#F5E8E6', '#962E2A', '#962E2A'); ?>
        </div>
        ...
        <div class="feature-body">
          ...
          <?php if (!empty(tek_str($feature, 'author', ''))): ?>
            <span class="blog-author">By <?php echo esc_html(tek_str($feature, 'author', '')); ?></span>
          <?php
    endif; ?>
          ...
          <div class="blog-footer-row">
            <?php if (!empty(tek_str($feature, 'date', ''))): ?>
              <span class="blog-date"><?php echo esc_html(tek_str($feature, 'date', '')); ?></span>
            <?php
    endif; ?>
    ...
    $item_text = trim(tek_str($item, 'text', '')); ?>
          <article class="item">
            ...
            <div class="item-data">
              <?php echo $build_tags($item, 'item', 'top', 'outline', '#F5E8E6', '#962E2A', '#962E2A'); ?>
              <h3><?php echo esc_html(tek_str($item, 'heading', 'Coming Soon')); ?></h3>
              <?php if (!empty(tek_str($item, 'author', ''))): ?>
                <span class="blog-author">By <?php echo esc_html(tek_str($item, 'author', '')); ?></span>
              <?php
    endif; ?>
              <?php if ($item_text !== ''): ?>
              ...
              <div class="blog-footer-row">
                <?php if (!empty(tek_str($item, 'date', ''))): ?>
                  <span class="blog-date"><?php echo esc_html(tek_str($item, 'date', '')); ?></span>
                <?php
    endif; ?>
                <?php echo $item_read; ?>
              </div>
